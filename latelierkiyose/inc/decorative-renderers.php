 ...
 *
 * Centralizes decorative SVG variants and returns rendered markup as strings
 * so template parts can stay declarative.
 *
 * @package Kiyose
 * @since   2.6.0
 */
 ...
 */
function kiyose_get_decorative_shape_variants(): array {
	return array(
...
/**
 * Returns available small decorative scribble variants.
 *
 * @return array<string, array<int, callable-string>>
 */
function kiyose_get_decorative_scribble_variants(): array {
	return array(
		'spiral'      => array( 'kiyose_render_spiral_1', 'kiyose_render_spiral_2' ),
		'squiggle'    => array( 'kiyose_render_squiggle_1', 'kiyose_render_squiggle_2' ),
		'mini-splash' => array( 'kiyose_render_mini_splash_1', 'kiyose_render_mini_splash_2' ),
		'virgules'    => array( 'kiyose_render_virgules_1', 'kiyose_render_virgules_2' ),
		'cross'       => array( 'kiyose_render_cross_1', 'kiyose_render_cross_2' ),
	);
}

/**
 * Returns available wave separator variants.
 *
 * @return array<string, array<int, callable-string>>
 */
function kiyose_get_wave_separator_variants(): array {
	return array(
		'wave' => array(
...
/**
 * Renders a large decorative shape SVG.
 *
 * @param array<string, mixed> $args Shape arguments.
 * @return string Rendered SVG or empty string for unknown types.
 */
function kiyose_render_decorative_shape_svg( array $args ): string {
	$kiyose_variants = kiyose_get_decorative_shape_variants();
	$kiyose_type     = kiyose_normalize_decorative_type( $args['type'] ?? 'splash' );

	if ( ! isset( $kiyose_variants[ $kiyose_type ] ) ) {
		return '';
	}

	$kiyose_color    = kiyose_normalize_decorative_color( (string) ( $args['color'] ?? '--kiyose-color-primary' ) );
	$kiyose_opacity  = kiyose_normalize_decorative_opacity( $args['opacity'] ?? 0.4 );
	$kiyose_size     = kiyose_normalize_decorative_size( $args['size'] ?? 'md', 'shape' );
	$kiyose_position = kiyose_normalize_decorative_position( (string) ( $args['position'] ?? 'top-left' ) );
	$kiyose_rotation = kiyose_normalize_decorative_rotation( $args['rotation'] ?? 0 );
	$kiyose_top      = kiyose_normalize_decorative_css_offset( $args['top'] ?? '' );
	...
	$kiyose_px       = $kiyose_size_map[ $kiyose_size ];
	...
	$kiyose_variant = kiyose_pick_decorative_variant( $kiyose_variants[ $kiyose_type ] );
	$kiyose_body    = kiyose_capture_decorative_variant( $kiyose_variant, array( $kiyose_color, $kiyose_opacity ) );

	return sprintf(
		'<svg class="%1$s" aria-hidden="true" role="presentation" width="%2$d" height="%2$d" viewBox="%3$s" style="%4$s" preserveAspectRatio="%5$s">%6$s</svg>',
		esc_attr( implode( ' ', $kiyose_classes ) ),
		$kiyose_px,
		esc_attr( $kiyose_viewbox_map[ $kiyose_type ] ),
		esc_attr( $kiyose_inline_style ),
		esc_attr( $kiyose_par_map[ $kiyose_type ] ),
		$kiyose_body
	);
}

/**
 * Renders a small decorative scribble SVG.
 *
 * @param array<string, mixed> $args Scribble arguments.
 * @return string Rendered SVG or empty string for unknown types.
 */
function kiyose_render_decorative_scribble_svg( array $args ): string {
	$kiyose_variants = kiyose_get_decorative_scribble_variants();
	$kiyose_type     = kiyose_normalize_decorative_type( $args['type'] ?? 'spiral' );

	if ( ! isset( $kiyose_variants[ $kiyose_type ] ) ) {
		return '';
	}

	$kiyose_color    = kiyose_normalize_decorative_color( (string) ( $args['color'] ?? '--kiyose-color-accent' ) );
	$kiyose_size     = kiyose_normalize_decorative_size( $args['size'] ?? 80, 'scribble' );
	$kiyose_rotation = kiyose_normalize_decorative_rotation( $args['rotation'] ?? 0 );
	$kiyose_float    = ! empty( $args['float'] );

	$kiyose_position_styles = array();
	foreach ( array( 'top', 'right', 'bottom', 'left' ) as $kiyose_prop ) {
		$kiyose_offset = kiyose_normalize_decorative_css_offset( $args[ $kiyose_prop ] ?? '' );
		if ( '' !== $kiyose_offset ) {
			$kiyose_position_styles[] = $kiyose_prop . ':' . $kiyose_offset;
		}
	}

	$kiyose_classes = array( 'deco-element' );
	if ( $kiyose_float ) {
		$kiyose_classes[] = 'deco-element--float';
	}

	$kiyose_viewbox_map = array(
		'spiral'      => '0 0 1280 1280',
		'squiggle'    => '0 -1 20 20',
		'mini-splash' => '0 0 1280 1225',
		'virgules'    => '0 0 90 100',
		'cross'       => '0 0 46.205 46.205',
	);
	$kiyose_style       = '--kiyose-deco-rotation: ' . $kiyose_rotation . 'deg';

	if ( $kiyose_position_styles ) {
		$kiyose_style .= ';' . implode( ';', $kiyose_position_styles );
	}

	$kiyose_variant = kiyose_pick_decorative_variant( $kiyose_variants[ $kiyose_type ] );
	$kiyose_body    = kiyose_capture_decorative_variant( $kiyose_variant, array( $kiyose_color ) );

	return sprintf(
		'<svg class="%1$s" aria-hidden="true" role="presentation" width="%2$d" height="%2$d" viewBox="%3$s" style="%4$s" preserveAspectRatio="xMidYMid meet">%5$s</svg>',
		esc_attr( implode( ' ', $kiyose_classes ) ),
		$kiyose_size,
		esc_attr( $kiyose_viewbox_map[ $kiyose_type ] ),
		esc_attr( $kiyose_style ),
		$kiyose_body
	);
}

/**
 * Renders a wave separator SVG.
 *
 * @param array<string, mixed> $args Separator arguments.
 * @return string Rendered separator or empty string for unknown types.
 */
function kiyose_render_wave_separator_svg( array $args ): string {
	$kiyose_variants = kiyose_get_wave_separator_variants();
	$kiyose_type     = kiyose_normalize_decorative_type( $args['type'] ?? 'wave' );

	if ( ! isset( $kiyose_variants[ $kiyose_type ] ) ) {
		return '';
	}

	$kiyose_fill_color = kiyose_get_decorative_css_color(
		kiyose_normalize_decorative_color( (string) ( $args['color'] ?? '--kiyose-color-background' ) )
	);
	$kiyose_bg_color   = kiyose_get_decorative_css_color(
		kiyose_normalize_decorative_color( (string) ( $args['from_bg'] ?? '#ffffff' ) )
	);

	$kiyose_variant = kiyose_pick_decorative_variant( $kiyose_variants[ $kiyose_type ] );
	$kiyose_body    = kiyose_capture_decorative_variant( $kiyose_variant, array( $kiyose_fill_color ) );

	return sprintf(
		'<div class="wave-separator" role="presentation" aria-hidden="true" style="background-color: %1$s;"><svg width="100%%" viewBox="0 0 1440 120" preserveAspectRatio="none" aria-hidden="true" focusable="false">%2$s</svg></div>',
		esc_attr( $kiyose_bg_color ),
		$kiyose_body
	);
}

/**
 * Normalizes decorative color values.
 *
 * @param string $value Candidate color.
 * @return string Safe CSS variable name or literal color.
 */
function kiyose_normalize_decorative_color( string $value ): string {
	$kiyose_value = trim( $value );

	if ( preg_match( '/^--kiyose-[a-z0-9-]+$/', $kiyose_value ) ) {
		return $kiyose_value;
	}

	if ( preg_match( '/^#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6}|[0-9a-fA-F]{8})$/', $kiyose_value ) ) {
		return $kiyose_value;
	}

	if ( preg_match( '/^rgba?\(\s*[0-9.%]+\s*,\s*[0-9.%]+\s*,\s*[0-9.%]+(?:\s*,\s*(?:0|1|0?\.\d+|[0-9]{1,3}%))?\s*\)$/', $kiyose_value ) ) {
		return $kiyose_value;
	}

	if ( preg_match( '/^rgb\(\s*[0-9.%]+\s+[0-9.%]+\s+[0-9.%]+(?:\s*\/\s*(?:0|1|0?\.\d+|[0-9]{1,3}%))?\s*\)$/', $kiyose_value ) ) {
		return $kiyose_value;
	}

	return '--kiyose-color-primary';
}

/**
 * Normalizes decorative opacity values.
 *
 * @param mixed $value Candidate opacity.
 * @return float Opacity bounded between 0 and 1.
 */
function kiyose_normalize_decorative_opacity( mixed $value ): float {
	if ( ! is_numeric( $value ) ) {
		return 0.4;
	}
...
/**
 * Normalizes decorative size values.
 *
 * @param mixed  $value  Candidate size.
 * @param string $family Size family.
 * @return string|int Safe size preset or pixel value.
 */
function kiyose_normalize_decorative_size( mixed $value, string $family ): string|int {
	if ( 'shape' === $family ) {
		$kiyose_size = (string) $value;

		if ( in_array( $kiyose_size, array( 'sm', 'md', 'lg' ), true ) ) {
			return $kiyose_size;
		}
	...
	$kiyose_size = is_numeric( $value ) ? (int) $value : 80;
	if ( $kiyose_size < 1 || $kiyose_size > 240 ) {
		return 80;
	}
...
/**
 * Normalizes decorative position class suffixes.
 *
 * @param string $value Candidate position.
 * @return string Safe position.
 */
function kiyose_normalize_decorative_position( string $value ): string {
	$kiyose_position = trim( $value );
	$kiyose_allowed  = array(
		'top-left',
		'top-right',
		'bottom-left',
		'bottom-right',
	...
	if ( in_array( $kiyose_position, $kiyose_allowed, true ) ) {
		return $kiyose_position;
	}
...
/**
 * Normalizes decorative type keys.
 *
 * @param mixed $value Candidate type.
 * @return string Safe key.
 */
function kiyose_normalize_decorative_type( mixed $value ): string {
	return strtolower( preg_replace( '/[^a-z0-9_-]/', '', (string) $value ) );
}

/**
 * Normalizes rotation values.
 *
 * @param mixed $value Candidate rotation.
 * @return int Rotation bounded to one full turn.
 */
function kiyose_normalize_decorative_rotation( mixed $value ): int {
	if ( ! is_numeric( $value ) ) {
		return 0;
	}

	return max( -360, min( 360, (int) $value ) );
}

/**
 * Normalizes CSS offsets used by decorative elements.
 *
 * @param mixed $value Candidate CSS offset.
 * @return string Safe CSS offset or empty string.
 */
function kiyose_normalize_decorative_css_offset( mixed $value ): string {
	$kiyose_value = trim( (string) $value );

	if ( '' === $kiyose_value ) {
		return '';
	}

	if ( '0' === $kiyose_value || preg_match( '/^-?\d+(?:\.\d+)?(?:%|px|rem|em|vh|vw)$/', $kiyose_value ) ) {
		return $kiyose_value;
	}
...
/**
 * Wraps CSS custom properties as usable CSS color values.
 *
 * @param string $value Safe CSS variable name or literal color.
 * @return string CSS color expression.
 */
function kiyose_get_decorative_css_color( string $value ): string {
	if ( str_starts_with( $value, '--kiyose-' ) ) {
		return 'var(' . $value . ')';
	}
...
/**
 * Picks one renderer from a variant pool.
 *
 * @param array<int, string> $variants Variant callbacks.
 * @return string Variant callback.
 */
function kiyose_pick_decorative_variant( array $variants ): string {
	$kiyose_variants = array_values( $variants );

	return $kiyose_variants[ array_rand( $kiyose_variants ) ];
}

/**
 * Captures output from legacy SVG variant functions.
 *
 * @param string            $variant   Variant callback.
 * @param array<int, mixed> $arguments Callback arguments.
 * @return string Captured SVG fragment.
 */
function kiyose_capture_decorative_variant( string $variant, array $arguments ): string {
	ob_start();
	call_user_func_array( $variant, $arguments );
...
if ( ! function_exists( 'kiyose_render_spiral_1' ) ) {
	/**
	 * Render spiral variant 1 — potrace-traced single spiral.
	 *
	 * @param string $color CSS variable name.
	 */
	function kiyose_render_spiral_1( $color ) {
		?>
		<g transform="translate(0,1280) scale(0.1,-0.1)" fill="var(<?php echo esc_attr( $color ); ?>)" stroke="none">
			<path d="M5195 12794 c-601 -35 -1062 -119 -1545 -280 -514 -172 -977 -437
-1435 -823 -126 -106 -418 -394 -539 -531 -849 -962 -1431 -2220 -1616 -3489
-210 -1442 172 -3114 1054 -4609 805 -1367 1942 -2386 3156 -2829 278 -102
574 -171 900 -210 200 -24 663 -24 880 0 958 107 1943 467 3455 1262 452 239
...
if ( ! function_exists( 'kiyose_render_spiral_2' ) ) {
	/**
	 * Render spiral variant 2 — potrace-traced scattered spiral fragments.
	 *
	 * @param string $color CSS variable name.
	 */
	function kiyose_render_spiral_2( $color ) {
		?>
		<g transform="translate(0,1223) scale(0.1,-0.1)" fill="var(<?php echo esc_attr( $color ); ?>)" stroke="none">
			<path d="M6927 12214 c-59 -19 -123 -80 -156 -148 -62 -133 -69 -371 -21 -736
5 -41 21 -181 35 -310 46 -424 64 -549 116 -820 60 -310 73 -344 155 -420 85
-78 184 -101 263 -60 59 29 122 107 152 186 54 140 59 208 59 788 0 557 -5
654 -50 909 -38 219 -123 392 -257 518 -102 97 -194 126 -296 93z"/>
...
if ( ! function_exists( 'kiyose_render_squiggle_1' ) ) {
	/**
	 * Render squiggle variant 1 - wavy horizontal scribble.
	 *
	 * @param string $color CSS variable name.
	 */
	function kiyose_render_squiggle_1( $color ) {
		?>
		<g transform="translate(-2,-3)" fill="none" stroke="var(<?php echo esc_attr( $color ); ?>)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
if ( ! function_exists( 'kiyose_render_squiggle_2' ) ) {
	/**
	 * Render squiggle variant 2 - more chaotic, looping.
	 *
	 * @param string $color CSS variable name.
	 */
	function kiyose_render_squiggle_2( $color ) {
		?>
		<g transform="translate(-2,-3) scale(-1,1) translate(-22,0)" fill="none" stroke="var(<?php echo esc_attr( $color ); ?>)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
			<path d="M3,13.5793719 C9.98914658,5.83454059 14.2442185,2.78592627 15.7652158,4.43352892 C18.0467117,6.90493289 7.55581053,16.1455344 9.47834357,17.954063 C11.4008766,19.7625917 16.9959382,11.5719148 19.0578414,12.4109285 C21.1197445,13.2499421 16.1152903,18.1722847 17.4055985,18.9997829 C18.2658039,19.5514483 19.3191667,19.0606734 20.5656867,17.527458"/>
		</g>
		<?php
	}
...
if ( ! function_exists( 'kiyose_render_mini_splash_1' ) ) {
	/**
	 * Render mini-splash variant 1 — hand-traced potrace paint splatter (from splat.svg).
	 *
	 * @param string $color CSS variable name.
	 */
	function kiyose_render_mini_splash_1( $color ) {
		?>
		<g transform="translate(0,1211) scale(0.1,-0.1)" fill="var(<?php echo esc_attr( $color ); ?>)" stroke="none">
			<path d="M4265 12096 c-115 -37 -252 -142 -296 -227 -57 -112 -30 -207 142
-495 154 -259 214 -408 228 -562 14 -158 -6 -275 -99 -582 -59 -198 -74 -285
-66 -405 10 -140 32 -280 68 -420 41 -163 41 -249 2 -312 -16 -25 -46 -60 -67
-77 -36 -28 -45 -31 -110 -31 -62 0 -80 5 -134 34 -134 72 -293 237 -434 450
...
if ( ! function_exists( 'kiyose_render_mini_splash_2' ) ) {
	/**
	 * Render mini-splash variant 2 — hand-traced potrace paint splatter with satellite droplets (from splat2.svg).
	 *
	 * @param string $color CSS variable name.
	 */
	function kiyose_render_mini_splash_2( $color ) {
		?>
		<g transform="translate(0,1223) scale(0.1,-0.1)" fill="var(<?php echo esc_attr( $color ); ?>)" stroke="none">
			<path d="M8217 12216 c-103 -38 -179 -124 -267 -301 -140 -280 -261 -765 -340
-1364 -32 -241 -38 -236 96 -65 667 853 946 1467 756 1667 -60 64 -169 92
...
if ( ! function_exists( 'kiyose_render_virgules_1' ) ) {
	/**
	 * Render virgules variant 1 - three short misaligned strokes.
	 *
	 * @param string $color CSS variable name.
	 */
	function kiyose_render_virgules_1( $color ) {
		?>
		<g fill="var(<?php echo esc_attr( $color ); ?>)">
			<path d="M20,30 C25,25 45,22 55,28 C60,32 58,38 50,40 C40,42 22,40 18,35 C16,33 17,31 20,30 Z"/>
			<path d="M25,52 C30,46 55,44 68,50 C74,53 72,60 62,62 C50,64 28,62 24,57 C22,55 22,53 25,52 Z"/>
			<path d="M18,72 C23,68 48,65 58,70 C63,73 60,79 52,80 C42,82 20,80 17,76 C15,74 15,73 18,72 Z"/>
		</g>
		<?php
	}
}

if ( ! function_exists( 'kiyose_render_virgules_2' ) ) {
	/**
	 * Render virgules variant 2 - two strokes, more tapered, different angles.
	 *
	 * @param string $color CSS variable name.
	 */
	function kiyose_render_virgules_2( $color ) {
		?>
		<g fill="var(<?php echo esc_attr( $color ); ?>)">
			<path d="M15,35 C22,28 50,24 65,32 C72,36 68,44 58,46 C45,48 18,44 14,38 C12,36 13,34 15,35 Z"/>
			<path d="M22,60 C28,54 58,52 72,58 C78,62 74,68 65,70 C52,72 24,68 20,63 C18,61 19,59 22,60 Z"/>
		</g>
		<?php
	}
}

if ( ! function_exists( 'kiyose_render_cross_1' ) ) {
	/**
	 * Render cross variant 1 - two thick crossed strokes, slightly off-center.
	 *
	 * @param string $color CSS variable name.
	 */
	function kiyose_render_cross_1( $color ) {
		?>
		<g fill="var(<?php echo esc_attr( $color ); ?>)" stroke="none">
...
if ( ! function_exists( 'kiyose_render_cross_2' ) ) {
	/**
	 * Render cross variant 2 - asterisk style, three strokes.
	 *
	 * @param string $color CSS variable name.
	 */
	function kiyose_render_cross_2( $color ) {
		?>
		<g fill="var(<?php echo esc_attr( $color ); ?>)" stroke="none" transform="scale(-1,1) translate(-46.205,0)">
			<path d="M26.485,43.39c-2.407,0-4.629-0.832-6.38-2.466c-2.729-2.545-3.581-6.527-2.172-10.146
				c1.862-4.779,5.877-8.689,10.63-10.619c-1.466-0.635-3.747-0.799-7.128,0.83c-0.452,0.218-1.107,0.615-1.801,1.036
				c-3.514,2.133-10.827,6.57-16.615,0.377c-2.452-2.623-3.481-6.221-2.826-9.871C0.917,8.5,3.522,5.094,7.339,3.185
				c1.733-0.863,3.832-0.163,4.696,1.565c0.864,1.729,0.163,3.831-1.565,4.696c-1.822,0.911-3.058,2.487-3.388,4.324
